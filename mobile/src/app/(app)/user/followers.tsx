...
export default function FollowersScreen() {
  ...
  return (
    <SafeAreaView testID="followers-screen" className="flex-1" style={{ backgroundColor: theme.bg }} edges={['top']}>
      {/* Header */}
      <View className="flex-row items-center px-4 py-3 gap-3" style={{ borderBottomColor: theme.border, borderBottomWidth: 0.5 }}>
        <Pressable testID="back-button" onPress={handleBack} className="p-1">
          <ArrowLeft size={22} color={theme.text} />
        </Pressable>
        <Text style={{ color: theme.text }} className="font-bold text-lg">{isFollowers ? 'Followers' : 'Following'}</Text>
      </View>
          ...
          renderItem={({ item }) => (
            ...
            >
              ...
              <View className="flex-1">
                <Text style={{ color: theme.text }} className="font-semibold text-sm">{item.name}</Text>
                {item.username ? (
                  <Text style={{ color: theme.subtext, fontSize: 13 }}>@{item.username}</Text>
                ) : null}
              </View>
            </Pressable>
          )}
